...
public class AutonomousBlueTrussPoses {
    ...
    {
        return mirrorColor(new Pose2d(-2*AutonomousUtils.TILE_WIDTH, -AutonomousUtils.TILE_WIDTH_HALF-3 , Math.toRadians(0)), color);
    }
    public static Pose2d getStack2Pose(AutonomousUtils.AllianceColor color, AutonomousUtils.StartPosition startPosition)
    {
        return mirrorColor(new Pose2d(-3*AutonomousUtils.TILE_WIDTH, -AutonomousUtils.TILE_WIDTH-4, Math.toRadians(0)).plus(new Twist2d(new Vector2d(15+6.5,0),mirrorColor(Math.toRadians(-15),color))),color);
    }
    public static Pose2d getBackdropPose(AutonomousUtils.AllianceColor color, AutonomousUtils.StartPosition startPosition, AutonomousUtils.AutoCase autoCase)
    {
        ...
        final double[] offset = new double[] {-3,3,7};
    ...
    public static Pose2d getBackdropExterior(AutonomousUtils.AllianceColor color, AutonomousUtils.StartPosition startPosition)
    {
        ...
        if(startPosition== AutonomousUtils.StartPosition.Truss)
            return mirrorColor(new Pose2d(2*AutonomousUtils.TILE_WIDTH,-AutonomousUtils.TILE_WIDTH-AutonomousUtils.TILE_WIDTH_HALF+5,Math.toRadians(0)),color);
        else return mirrorColor(new Pose2d(2*AutonomousUtils.TILE_WIDTH,-AutonomousUtils.TILE_WIDTH-AutonomousUtils.TILE_WIDTH_HALF-4,Math.toRadians(0)),color);
